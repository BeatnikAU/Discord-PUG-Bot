...
bot.on('message', message => {
    ...
    if (/!teamsize [1-4]/i.test(message.content)) {
    ...
    if (/!jointime [1-9]/i.test(message.content)) {
        if (message.content.length == 11) {
...
var commands = {
    ...
    startMatch: function(div, author) {
        ...
        for (i = 0; i < divA.team1.length; i++) {
            let player = divA.team1[i].toString().slice(2, -1);
        ...
        for (i = 1; i < divA.team2.length; i++) {
            let player = divA.team2[i].toString().slice(2, -1);
